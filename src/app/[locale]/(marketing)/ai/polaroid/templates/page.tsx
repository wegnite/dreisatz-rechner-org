...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: 'AIPolaroidTemplatesPage' });

  return constructMetadata({
    title: t('title'),
    description: t('description'),
    canonicalUrl: getUrlWithLocale('/ai/polaroid/templates', locale),
  });
}
...
export default async function AIPolaroidTemplatesPage({ params }: PageProps) {
  const { locale } = await params;
  const t = await getTranslations({ locale, namespace: 'AIPolaroidTemplatesPage' });
            ...
            {hero.primaryCta ? (
              <div className="pt-4">
                <Button size="lg" asChild>
                  <Link href={templatesHref}>{hero.primaryCta}</Link>
                </Button>
              </div>
            ) : null}
          </div>
